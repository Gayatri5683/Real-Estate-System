...
import {
  maharashtraCities,
  cityDescriptions,
} from "./constants/maharashtraCities";
...
const AddProperty = () => {
  ...
  useEffect(() => {
    if (formNewData.location && cityDescriptions[formNewData.location]) {
      setFormNewData((prev) => ({
        ...prev,
        description: cityDescriptions[formNewData.location],
      }));
    }
  }, [formNewData.location]);

  const handleChange = (e) => {
    const { name, value } = e.target;

    // Handle nested objects in formNewData
    if (name.includes(".")) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form
    if (!formNewData.name || !formNewData.location || !formNewData.price) {
      toast({
    ...
    console.log("nearByPlaces", formNewData.nearbyPlaces);

    // Add property
    const newProperty = {
      id: Date.now(), // Generate a unique ID
      name: formNewData.name,
      location: formNewData.location,
      address: formNewData.address,
      coordinates: formNewData.coordinates,
      propertyType: formNewData.propertyType,
      price: parseFloat(formNewData.price),
      description: formNewData.description,
      bedrooms: parseInt(formNewData.bedrooms) || 1,
      bathrooms: parseInt(formNewData.bathrooms) || 1,
      area: parseFloat(formNewData.area) || 0,
      images: imageUrls,
      mainImage: imageUrls[0], // First image as the main image
      amenities: formNewData.amenities,
      nearbyPlaces: formNewData.nearbyPlaces,
      type: "featured", // Set the property type as featured
      seller: {
        id: currentUser.id,
    ...
    formData.append("name", formNewData.name);
    formData.append("location", formNewData.location);
    formData.append("address", JSON.stringify(formNewData.address));
    formData.append("coordinates", JSON.stringify(formNewData.coordinates)); // If coordinates is an object, you might need to JSON.stringify it: JSON.stringify(formNewData.coordinates)
    formData.append("propertyType", formNewData.propertyType);
    formData.append("price", parseFloat(formNewData.price));
    formData.append("description", formNewData.description);
    formData.append("bedrooms", parseInt(formNewData.bedrooms) || 1);
    formData.append("bathrooms", parseInt(formNewData.bathrooms) || 1);
    formData.append("area", parseFloat(formNewData.area) || 0);

    // Append images (assuming 'uploadedImages' is an array of File objects)
    images.forEach((image, index) => {
      console.log("upload image", image);
      formData.append(`images`, image); // Backend should handle multiple files with the same key
    });

    // // Append amenities (assuming formNewData.amenities is an array)
    // formNewData.amenities.forEach((amenity) => {
    //   formData.append("amenities[]", amenity); // Use '[]' for arrays to help backend frameworks parse it
    // });

    formData.append("amenities", formNewData.amenities.join(", "));
    formData.append("nearbyPlaces",JSON.stringify(formNewData.nearbyPlaces));

    formData.append("type", "featured");

    // Append seller information. For nested objects, you typically stringify them or append each property individually.
    // Stringifying is often simpler for complex nested objects if your backend can parse JSON strings within FormData.
    formData.append(
      "seller",
      JSON.stringify({
        id: currentUser.id,
    ...
    formData.append("listedDate", new Date().toISOString());

    // Add to context
    if (addProperty) {
      const url = "http://localhost:3000/api/v1/properties/add";
  ...
  return (
    <div className="overflow-hidden">
      ...
      <div
        style={{ maxWidth: 1000 }}
        className="mx-auto bg-white rounded-lg shadow-lg p-8 border border-amber-100 my-10"
      >
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-amber-800">
            List Your Maharashtra Property
          </h2>
          ...
          <div className="bg-amber-50 p-6 rounded-lg border border-amber-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col">
                ...
                <select
                  name="propertyType"
                  value={formNewData.propertyType}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
                  required
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col">
                ...
                <input
                  type="text"
                  name="address.street"
                  value={formNewData.address.street}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.address.locality}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.address.landmark}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.address.pincode}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    value={formNewData.coordinates.latitude}
                    onChange={handleChange}
                    className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
                  ...
                  <input
                    ...
                    value={formNewData.coordinates.longitude}
                    onChange={handleChange}
                    className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.nearbyPlaces.schools}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.nearbyPlaces.hospitals}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.nearbyPlaces.markets}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  value={formNewData.nearbyPlaces.transportation}
                  onChange={handleChange}
                  className="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-amber-500"
              ...
              {amenitiesList.map((amenity) => (
                <div key={amenity} className="flex items-center">
                  <input
                    type="checkbox"
                    id={`amenity-${amenity}`}
                    checked={formNewData.amenities.includes(amenity)}
          ...
          <div className="bg-amber-50 p-6 rounded-lg border border-amber-200">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Description will be auto-filled based on selected city, but you
                can customize it.
              </p>
            </div>
          </div>
